...
void DCS::process_udp()
{
    ...
    {
        emit update_log("Connected to DCS");
...
//			qDebug() << "New stream from " << m_modeinfo.src << " to " << m_modeinfo.dst << " id == " << QString::number(m_modeinfo.streamid, 16);
        ...
        if (buf.data()[45] & 0x40)
        {
            emit update_log("DCS RX stream ended");
			m_rxwatchdog = 0;
        ...
        if (m_modeinfo.stream_state == STREAMING)
        {
            if (m_modem)
            {
				m_rxmodemq.append(MMDVM_FRAME_START);
...
void DCS::process_rx_data()
{
    ...
    if (m_rxwatchdog++ > 100)
    {
        emit update_log("DCS RX stream timeout");
		m_rxwatchdog = 0;
    ...
    //		emit update_output_level(m_audio->level());
        ...
        if ((m_modeinfo.stream_state == STREAM_END) || (m_modeinfo.stream_state == STREAM_LOST))
        {
            ...
            m_modeinfo.streamid = 0;
            m_rxcodecq.clear();
            emit update_log("DCS playback stopped");
            m_modeinfo.stream_state = STREAM_IDLE;
            return;
        }
}
