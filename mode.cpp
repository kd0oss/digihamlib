...
void Mode::init(QString callsign,
                uint32_t dmrid,
                uint16_t nxdnid,
                char module,
                QString refname,
...
void Mode::ambe_connect_status(bool s)
{
    ...
    else
    {
        ...
        emit update_log("AMBE connect failed");
	}
...
void Mode::mmdvm_connect_status(bool s)
{
	if(s){
        ...
        emit update_log("mmdvm connect failed");
    }
    emit update(m_modeinfo);
}
...
{
    if (m_mdirect && (m_mode == "M17"))
    { // MMDVM_DIRECT currently only supported by M17
        mmdvm_direct_connect();
    }
    else
        if (m_ipv6 && (m_modeinfo.host != "none"))
        {
            emit update_log(QString("Host = %1").arg(m_modeinfo.host));
