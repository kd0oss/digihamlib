...
//			m_audio->init();
            ...
            { //OKRW
                ...
                if (buf.data()[7] == 0x4f)
                { //OKRO -- Go get registered!
                    m_modeinfo.status = CONNECTED_RW;
                }
		}
        else
            if ((buf.data()[4] == 0x46) && (buf.data()[5] == 0x41) && (buf.data()[6] == 0x49) && (buf.data()[7] == 0x4c))
            { // FAIL response
...
//                m_audio->start_playback();
                ...
                emit update_log(QString("New stream from [%1] to [%2] id = %3").arg(m_modeinfo.src).arg(m_modeinfo.dst).arg(QString::number(m_modeinfo.streamid, 16)));
                emit update(m_modeinfo);
                sd_gps_cnt = 0;
                gps_data.clear();
			}
    ...
    if ((buf.size() == 0x1d) && (!memcmp(buf.data()+1, header, 5)) )
    { //29
		const uint16_t streamid = (buf.data()[14] << 8) | (buf.data()[15] & 0xff);
    ...
    { //32
        ...
        if (streamid == m_modeinfo.streamid)
        {
            ...
            emit update_log("REF RX stream ended");
			m_rxwatchdog = 0;
...
void REF::process_rx_data()
{
    ...
    if (m_rxwatchdog++ > 50)
    {
        emit update_log("REF RX stream timeout");
		m_rxwatchdog = 0;
    ...
    if ((!m_tx) && (m_rxcodecq.size() > 8) )
    {
        ...
        if ((m_modeinfo.stream_state == STREAM_END) || (m_modeinfo.stream_state == STREAM_LOST))
        {
            ...
            m_modeinfo.streamid = 0;
            m_rxcodecq.clear();
            emit update_log("REF playback stopped");
            m_modeinfo.stream_state = STREAM_IDLE;
            return;
        }
}
